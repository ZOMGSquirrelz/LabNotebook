...
def multiple_in_range_calc(result_stats):
    ...
    for dil, result in result_stats:
        ...
        diluted_values.append(result)       # Adds result to the diluted_values list
...
# Compares the results to the specific countable range for the test and outputs the final result
...
# Perform rounding rules to chemistry result entries
...
def generate_report_results(project_id):
    ...
    for sample in sample_numbers:
        ...
        for result in results_list:
            if result[0] == sample and (result[1] in config.petrifilm_tests or result[1] in config.chemistry_tests):
                sample_results.append([result[1], result[2]])
